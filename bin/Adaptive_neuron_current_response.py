...
tf.app.flags.DEFINE_integer('seq_len', 5000, 'Number of character steps')
...
tf.app.flags.DEFINE_float('input_current', 0.03, 'input current to the adaptive neuron')
tf.app.flags.DEFINE_float('injected_noise_current', 0.03, 'input current to the adaptive neuron')
##
tf.app.flags.DEFINE_bool('tau_a_spread', False, 'Mikolov model spread of alpha - threshold decay')
tf.app.flags.DEFINE_bool('save_data', False, 'Save the full trajectory data on the testing set')
...
tf.app.flags.DEFINE_bool('sine', False, 'Modulate the input current with a sine wave')
...
class ALIFv(ALIF):
    ...
    def __call__(self, inputs, state, scope=None, dtype=tf.float32):
        ...
        return [new_z, new_b, new_v, input_current], new_state
...
file_reference = '{}_{}_in{}_R{}_A{}__Iin{:.0f}_V0{:.0f}_comment{}'.format(
